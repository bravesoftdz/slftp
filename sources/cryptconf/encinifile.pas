...
uses {$IFDEF MSWINDOWS}System.Classes{$ELSE}Classes{$ENDIF}, slmd5, {$IFDEF MSWINDOWS}System.SyncObjs{$ELSE}SyncObjs{$ENDIF};
...
type
  ...
  public
    AutoUpdate: Boolean;
    constructor Create(const FileName:string; const Passphrase: ansistring; autoupdate: Boolean = False; compression: Boolean = True); overload;
...
uses {$IFDEF MSWINDOWS}System.SysUtils{$ELSE}SysUtils{$ENDIF}, slblowfish;
...
constructor TEncIniFile.Create(const FileName: string; const Passphrase: ansistring; autoupdate: Boolean = False; compression: Boolean = True);
begin
  if passPhrase = '' then
    fSima:= True;
